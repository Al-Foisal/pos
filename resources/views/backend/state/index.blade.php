            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <!-- /.card-header -->
                        <div class="card-body">
                            <a href="{{ route('admin.state.create') }}" class="btn btn-outline-primary">Add State</a>
                            <br>
                            <br>
                                    ...
                                    @foreach ($states as $item)
                                        <tr>
                                            <td class="d-flex justify-content-around">
                                                <a href="{{ route('admin.state.edit', $item) }}"
                                                    class="btn btn-info btn-xs"> <i class="fas fa-edit"></i> Edit</a>
                                                @if ($item->status === 1)
                                                    <form action="{{ route('admin.state.inactive', $item) }}"
                                                        method="post">
                                                        @csrf
                                                        <button type="submit"
                                                ...
                                                @else
                                                    <form action="{{ route('admin.state.active', $item) }}" method="post">
                                                        @csrf
                                                        <button type="submit"
